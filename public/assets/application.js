(function() {
  ...
  $(function() {
    ...
    MP.Players.on('reset', function(coll) {
      return MP.User = coll.detect(function(player) {
        return player.get('player') === true;
      });
    });
    MP.mediator.on('devicemotion', function(data) {
      if (!MP.User) return false;
      MP.User.set('move', {
        horiz: data.horiz,
        vert: data.vert
      });
    ...
    return $(document).on('keyup', function(event) {
      var key, moves;
      if (!(MP.User && valid_keys.indexOf(event.which) !== -1)) return false;
      key = event.which;
      moves = {};
      if (key === 37 || key === 39) {
        moves.horiz = key === 37 ? 'L' : 'R';
      } else {
        moves.vert = key === 38 ? 'U' : 'D';
      }
...
(function() {
  ...
  Motion = (function() {
    ...
    Motion.prototype.subscribers = {};

    function Motion() {
      this.onOrientationChange = __bind(this.onOrientationChange, this);
      this.onDeviceMotion = __bind(this.onDeviceMotion, this);      window.addEventListener('devicemotion', this.onDeviceMotion, false);
      $(window).on('orientationchange', this.onOrientationChange);
    }

    Motion.prototype.on = function(type, callback) {
      var handlers, _base;
      handlers = this.subscribers[type];
      if (_(handlers).include(callback)) return;
      (_base = this.subscribers)[type] || (_base[type] = []);
      this.subscribers[type].push(callback);
      return MP.mediator.on(type, callback, this);
    };

    Motion.prototype.off = function(type, callback) {
      var handlers, index;
      handlers = this.subscribers[name];
      if (handlers) {
        index = _(handlers).indexOf(handler);
        if (index > -1) handlers.splice(index, 1);
        if (handlers.length === 0) delete this.subscribers[type];
      }
      return MP.mediator.off(type, handler);
    };

    Motion.prototype.cleanup = function() {
      var handler, handlers, type, _ref, _results;
      ...
      for (type in _ref) {
        ...
        _results.push((function() {
          ...
          for (_i = 0, _len = handlers.length; _i < _len; _i++) {
            handler = handlers[_i];
            _results2.push(MP.mediator.unsubscribe(type, handler));
          }
          return _results2;
        })());
      }
    ...
    Motion.prototype.onOrientationChange = function(event) {
      var orient;
      orient = Math.abs(window.orientation) === 90 ? 'landscape' : 'portrait';
      return MP.mediator.trigger('orientationchange', [orient]);
    };
...
(function() {
  ...
  Socket = (function() {
    ...
    Socket.prototype.connection = null;

    Socket.prototype.channel = null;

    function Socket(options) {
      this.send = __bind(this.send, this);
      this.receive = __bind(this.receive, this);      this.connection || (this.connection = new WebSocket("ws://" + options.host + ":" + options.port + "/" + options.path));
      this.channel = "root";
    ...
    Socket.prototype.receive = function(message) {
      ...
      return MP.mediator.trigger("receive", [message.data]);
    };
...
(function() {
  ...
  Players = (function(_super) {
    ...
    Players.prototype.connection = null;

    Players.prototype.sync = function() {
      return this.connection.send(JSON.stringify(this.toJSON()));
    };
    ...
    Players.prototype.receive = function(msg) {
      var data;
      data = JSON.parse(msg);
...
(function() {
  ...
  Player = (function(_super) {
    ...
    Player.prototype.tagName = 'span';

    Player.prototype.className = 'player';

    Player.prototype.initialize = function() {
      return this.model.on('change:x change:y', this.move);
    };
    ...
    Player.prototype.render = function() {
      ...
      if (this.model.get('player')) this.$el.addClass("self");
      return this.move();
    };
